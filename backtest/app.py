...
from tab_polymarket_scanner import render_polymarket_scanner_tab
...
with st.sidebar:
    ...
    since = st.text_input(
        ...
        help="Start date for the OHLCV data download from Binance. Earlier dates = more events but slower first load. Data is cached locally as Parquet.",
    )
    ...
    lookback = st.slider(
        ...
        help=(
            "Rolling window (in calendar days) for the daily log-return std dev used to compute monthly vol. "
            "360 days matches the Pine Script TradingView indicator. "
            "Formula: monthly_vol = rolling_std(log(hlcc4), lookback) × √365 / √12."
        ),
    )
    max_k = st.slider(
        "Max Holding Period (4H bars)", 30, 1080, 120, step=6,
        help=(
            "Maximum forward-return horizon computed per event. "
            "6 bars = 1 day  |  120 = 20d  |  360 = 60d  |  1080 = 180d. "
            "Higher values give more context but increase computation time."
        ),
    )
    ...
    sigmas_selected = st.multiselect(
        ...
        default=ZONE_SIGMAS,
        help=(
            "Which volatility multiples to compute and display zones for. "
            "Zone = monthly_open × (1 ± σ × monthly_vol). "
            "Supply zones are above monthly_open; demand zones are below."
        ),
    )
    ...
    with st.spinner(f"Loading data for **{symbol}**..."):
        try:
            ...
            st.session_state.update({
                ...
                "summary": summary,
                "current_monthly_vol": _current_monthly_vol,
            })
...
# Extrair DataFrames do Vol Zone Study (None se o estudo ainda não foi rodado)
_df_hitrate = st.session_state.get("summary")    # summarize_events output
_df_events  = st.session_state.get("study")      # raw events com colunas fwd_k

_main_tab, _scanner_tab = st.tabs(["📈 Vol Zone Study", "🎯 Polymarket Scanner"])

with _scanner_tab:
    render_polymarket_scanner_tab(
        df_hitrate = _df_hitrate,
        df_returns = _df_hitrate,   # mesmo DF — sharpe/mean_ret também estão aqui
        df_events  = _df_events,
    )
...
with _main_tab:
    ...
    with tab_price:
        ...
        st.caption(
            "4H candlestick chart with monthly volatility zones overlaid. "
            "Zones are fixed for the entire month and reset on the 1st bar of each new month. "
            "Triangles mark the first bar that touched each zone in that month."
        )
        ...
        with pc_c2:
            pc_sigmas = st.multiselect(
                ...
                key="pc_sigmas",
                help="Filter which σ zone levels are drawn. Supply zones (orange) above monthly open, demand zones (blue) below.",
            )
            pc_touches = st.toggle(
                "Show touch events", value=True, key="pc_touches",
                help="Overlay triangle markers at the first 4H bar each month where price wicked into each zone.",
            )
    ...
    with tab_curves:
        ...
        st.caption(
            "Mean log forward return after touching each zone, measured at k bars after the touch. "
            "Supply: negative return = favorable (price fell after reaching the supply zone). "
            "Demand: positive return = favorable (price rose after reaching the demand zone). "
            "Band = ±1 std dev across all events at that k. "
            "Formula: fwd_k = log(close[t+k] / close[t])."
        )
        c1, c2 = st.columns(2)
        with c1:
            st.plotly_chart(
    ...
    with tab_hit:
        ...
        st.caption(
            "Fraction of touch events where price moved in the expected direction at holding period k. "
            "Supply hit = price fell (fwd_k < 0). Demand hit = price rose (fwd_k > 0). "
            "50% = random baseline (coin flip). "
            "Sustained divergence above/below 50% indicates a statistically meaningful edge."
        )
        c1, c2 = st.columns(2)
        with c1:
    ...
    with tab_sharpe:
        ...
        st.caption(
            "Per-trade Sharpe ratio as a function of holding period k. "
            "Formula: mean(signed_return) / std(signed_return) where signed = −1 × fwd_k for supply, +1 × fwd_k for demand. "
            "Peak k = empirically optimal exit for each σ level. "
            "Use this to calibrate the holding period fed into the Polymarket Scanner."
        )
        c1, c2 = st.columns(2)
        with c1:
    ...
    with tab_dist:
        ...
        st.caption(
            "Violin plot of log forward returns across all events at a chosen holding period k. "
            "Box inside = interquartile range. White dot = median. Width = kernel density. "
            "Outlier dots shown beyond the whiskers. Wider violin = higher variance in outcomes."
        )
        ...
        k_for_dist = st.select_slider(
            ...
            help="Select how many 4H bars after the touch event to measure the return. 6 bars = 1 day.",
        )
